...
makePdSubgraph <- function(g,end,pdAttr='founded_at',acqPdAttr='acquired_at',closedPdAttr='closed_at')
{
  ...
  if(acqPdAttr %in% vertexAttrs) {
    ...
    acqPdVids <- V(g.sub)[which( !is.na(tmp) & tmp < end)]
    keepVids <- c(keepVids, acqPdVids )    
  }
  ##  REMOVE VERTICES (EDGES) IF   CLOSED BEFORE `end`
  if(closedPdAttr %in% vertexAttrs) {
      tmp <- igraph::get.vertex.attribute(g,closedPdAttr) 
      closedPdVids <- V(g.sub)[which( !is.na(tmp) & tmp < end)]
      keepVids <- c(keepVids, closedPdVids )    
  }
  ## INDUCE SUBGRAPH
  g.sub <- igraph::induced.subgraph(graph=g,vids=unique(keepVids))
  return(g.sub)
}
###
...
co <- merge(co,tmp, by.x='company_name_unique',by.y='acquired_name_unique', all.x=T,all.y=F)
