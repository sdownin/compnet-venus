...
acqd <- 1
mapping <- seq(1,6)
mapping[mapping==acqd] <- acqr
w2 <- igraph::contract(w,mapping = mapping, vertex.attr.comb=vertCombNames)
plotNet(w2)
...
#
##
getAcqContractedGraph(g,acquirer,acquired)

### 
# KEEP NAME OF AQUIRER WHEN CONTRACTING VERTICES FOR ACQUISITION `vertex.attr.comb`
##
vertCombNames <- function(x, concat=FALSE)
{
  if(concat) {
    out <- ifelse(length(x)==1,x,sprintf('%s (%s)',x[which(x==acqr)],paste(x[which(x!=acqr)],collapse=','))) 
  } else {
    out <- ifelse(length(x)==1,x,x[which(x==acqr)])
  }
  return(out)
}
