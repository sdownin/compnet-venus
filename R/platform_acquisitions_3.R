...
panel.smooth<-function (x, y, col = "blue", bg = NA, pch = 18,cex = 0.8, 
...
getAcqCountsByPeriod <- function(count.df, start, end, to.merge.df, 
                                 count.name='company_name_unique', new.field='acq_count', pdName='acquired_year')
{
  if(!(count.name%in%names(count.df)))
    return(NA)
  df.sub <- acq[which(acq[,pdName]>=start & acq[,pdName]<end), ]
  df.count <- count(df.sub[,count.name])
  names(df.count) <- c(count.name,new.field)
  to.merge.df <- merge(to.merge.df,df.count,by=count.name,all=T)
  to.merge.df[is.na(to.merge.df[,new.field]),new.field] <- 0
  return(to.merge.df)
}
###
# CREATE GRAPH FROM COMPETITIVE RELATIONS IN `comp` DATAFRAME
##
makeGraph <- function(end,comp,vertdf,name='company_name_unique', 
                      compName='competitor_name_unique', relationPdName='relation_created_at',
                      datesVec=c('founded_at','founded_month','founded_quarter','founded_year','acquired_at') )
{
...
###
# CREATE SUBGRAPH WITH VERTICES EXISTING BEFORE `end` DATE
#       FROM  GRAPH BUILT WITH makeGraph()
##
makePdSubgraph <- function(g,end,pdAttr='founded_at')
{
  # g.sub <- igraph::delete_edges(graph=g,edges = E(g)[which(E(g)$relation_created_at>end )])
  g.sub <- igraph::induced.subgraph(graph=g,vids=V(g)[which(igraph::get.vertex.attribute(g,pdAttr) < end)])
    ##
    ##  REMOVE VERTICES (EDGES) IF VERTEX WAS CLOSED, IPO, ACQUIRED BEFORE `end` date for period
    ##
  return(g.sub)
}
###
...
##-------------------------------------------------------
##  ADD CLOSED / ACQUIRED DATES TO REMOVE COMPANIES FROM COMPETITION NETWORK
##-------------------------------------------------------
tmp <- acq[,c('acquired_name_unique','acquired_at','company_name_unique')]
names(tmp) <- c('acquired_name_unique','acquired_at','acquired_by_company_name_unique')
co.c <- merge(co.c,tmp, by.x='company_name_unique',by.y='acquired_name_unique', all=T)
...
# TO DO:
#
#  REMOVE COMPANIES (closed, IPO)
#  ADD ACQUIRED CO's COMPETITORS INTO ACQUIRER COMPANY's EGONETWORK
#
#
########################################################################
...
periods <- c(0,seq(1999,2015,yrpd))
acq.l <- list()
lcc.l <- list()
g <- makeGraph(end=periods[t],comp=comp,vertdf=co.c)

for(t in 1:(length(periods)-1)) {
  df.tmp <- getAcqCountsByPeriod(count.df=acq, count.name='company_name_unique', new.field='acq_count', 
                                     start=periods[t], end=periods[t+1], to.merge.df = co.c)  
  df.tmp$period <- as.factor(periods[t+1])
  ##--------------
  ## CREATE PERIOD GRAPH
  ##-------------
  ...
  lcc.l[[t]] <- getNetworkProperties(lcc)
  ## GET DATAFRAME OF NETWORK ATTRIBUTES
  df.net <- getNetworkPropertiesDataframe(lcc.l[[t]])
  ##---------------
  ## MERGE NETWORK VARIABLES INTO PERIOD REGRESSION DATAFRAME
  ##---------------
  df.tmp <- merge(df.tmp,df.net,by='company_name_unique',all=T)
  acq.l[[t]] <- df.tmp
}; names(acq.l) <- periods[-1]  # name each period by ending year (remove first start year)
