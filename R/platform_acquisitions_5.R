...
for(t in 2:length(periods)) {
  df.tmp <- getAcqCountsByPeriod(count.df=acq, count.name='company_name_unique', new.field='acq_count', 
                                 start=periods[t-1], end=periods[t], to.merge.df = df.in)  
  df.tmp$period <- as.factor(periods[t])
  ##--------------
  ## CREATE PERIOD GRAPH
  ##-------------
  ...
  ##---------------
  acq.l[[t-1]] <- merge(df.tmp,df.net,by='company_name_unique',all=T)
  ## 
...
df.panel.reg <- df.panel
df.panel.reg$company_name_unique <-  as.factor(df.panel.reg$company_name_unique)
df.panel.reg$period <-  as.factor(df.panel.reg$period)
#
reg.vars <- c('acq_count','ego_size','ego_density','period','company_name_unique')
df.panel.reg <- na.omit(df.panel.reg[,which(names(df.panel.reg)%in%reg.vars)])
#
dim(df.panel.reg)
#
formula <- acq_count ~ ego_size + ego_density + (1|company_name_unique)+(1|period)
glmmADMB::glmmadmb(formula=formula, data=df.panel.reg, family="nbinom", link="log",
                   #start,
                   corStruct="diag", easyFlag=TRUE,
                   zeroInflation=FALSE, 
                   admb.opts=admbControl(maxfn=1000),
                   mcmc=FALSE,
                   mcmc.opts=mcmcControl(mcmc = 1000, mcmc2=0, mcnoscale = FALSE, mcgrope = FALSE, mcmult = 1),
                   save.dir=getwd(),
                   verbose=FALSE,
                   #extra.args="",
                   bin_loc=NULL,debug=TRUE)
