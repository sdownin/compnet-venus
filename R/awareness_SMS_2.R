...
fits1 <- readRDS('tergm_fits_cem_compare_new_m5_pow_cem_pd6_R200.rds')
# saveRDS(fits1, file = 'tergm_fits_cem_compare_new_m5_pow_cem_pd6_R200.rds')

fits2 <- readRDS('tergm_fits_cem_compare2_qualtrics_new_m5_pow_cem_pd6_R200.rds')

# fitsc <- readRDS('sms_tergm_new_gi_cem_clarabridge_fits.rds')
...
fits[[net_group]][[firm_i]]$m1@boot$t <- as.numeric(fits[[net_group]][[firm_i]]$m1@boot$t[-rows, ])

screenreg(fits$cem$clarabridge[c(1,2,3,4,5)])
...
#   nodecov("age") + absdiff("age") + 
#   edgecov(mmc) +
#   memory(type = "stability", lag = 2) + 
#   nodecov("cent_pow_n1_5") + absdiff("cent_pow_n1_5") + 
#   cycle(3) + cycle(4) + cycle(5) + 
#   nodecov("genidx_multilevel")
# 
# m4t <-   nets ~ edges + gwesp(0, fixed = T) + 
#   nodematch("ipo_status", diff = F) + 
#   nodematch("state_code", diff = F) + 
#   nodecov("age") + absdiff("age") + 
#   edgecov(mmc) +
...
# m4t2 <-   nets ~ edges + gwesp(0, fixed = T) + 
#   nodematch("ipo_status", diff = F) + 
#   nodematch("state_code", diff = F) + 
#   nodecov("age") + absdiff("age") + 
#   edgecov(mmc) +
...
#   cycle(3) + cycle(4) + cycle(5) + 
#   nodecov("genidx_multilevel") + 
#   timecov(transform = function(t) t) + timecov('genidx_multilevel', transform = function(t) t)
# 
...
#   edgecov(mmc)  + # edgecov(ldv) +   edgecov(smt)  +
#   memory(type="stability",lag=1) + 
#   nodecov('cent_pow_n3_0_lag')  + absdiff('cent_pow_n3_0_lag') +
#   cycle(3) + cycle(4) + cycle(5) + 
#   nodecov('genidx_multilevel_lag')
...
# fits[[net_group]][[firm_i]]$m4  <- btergm(m4,    R=R, parallel = "multicore", ncpus = detectCores()); summary(fits[[net_group]][[firm_i]]$m4)
# fits[[net_group]][[firm_i]]$m4l2 <- btergm(m4l2, R=R, parallel = "multicore", ncpus = detectCores()); summary(fits[[net_group]][[firm_i]]$m4l2)
# fits[[net_group]][[firm_i]]$m4t <- btergm(m4t,   R=R, parallel = "multicore", ncpus = detectCores()); summary(fits[[net_group]][[firm_i]]$m4t)
