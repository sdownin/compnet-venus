...
for (i in 1:length(files))
  ...
  m4 <-   nets.sub ~ edges + gwesp(0, fixed=T) +
    ...
    edgecov(mmc) + edgecov(smt)  + # edgecov(ldv) +
  ...
  fits[[net_group]][[firm_i]]$m11 <- btergm(m7, R=R, parallel = "multicore", ncpus = detectCores()); summary(fits[[net_group]][[firm_i]]$m11)
  fits[[net_group]][[firm_i]]$m12 <- btergm(m8, R=R, parallel = "multicore", ncpus = detectCores()); summary(fits[[net_group]][[firm_i]]$m12)
