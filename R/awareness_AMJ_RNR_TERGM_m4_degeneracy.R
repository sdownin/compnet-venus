...
fits <- fits.all[[name_i]][['m4_no_cpa']]
...
## size, density trends
grow <- sapply(nets[2:length(nets)], function(net){
  g <- asIgraph(net);
  e <- ecount(g)
  v <- length(V(g)[igraph::degree(g)>0])
  return(c(e=e,v=v,k=round(e/v,2)))
})
grow.df <- as.data.frame(t(grow))
print(grow.df)

## non-zero cooperative relations too sparse
cp <- sapply(nets[2:length(nets)], function(net){
  x <- net %n% 'coop'
  nz <- x[x > 0]
  return(c(nz=length(nz),pct_nz=round(100*length(nz)/length(x),4)))
})
cp.df <- as.data.frame(t(cp))
print(cp)
par(mfrow=c(1,1))
plot(cp.df$pct_nz)
...
for (effect in effects) {
  ...
  hist(x, main=effect, breaks=53, col=rgb(.2,.2,.2,.2), xlim=c(xl1,xl2))
...
interp.df.file <- sprintf('%s/interpret_winlocal_v2_%s_pd%s_d%s_R%s_%s.csv', 
