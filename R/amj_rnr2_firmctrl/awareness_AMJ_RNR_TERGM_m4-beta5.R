...
centjoin <- lapply(nets, function(net) as.matrix(net %n% 'joint_cent_pow_n0_5'))
centratio <- lapply(nets, function(net) as.matrix(net %n% 'cent_ratio_pow_n0_5'))
...
m4_beta5 <-   nets ~ edges + gwesp(0, fixed = T) + gwdegree(0, fixed=T) + 
