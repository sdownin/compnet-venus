...
## BUILD NETS LIST FROM INDIVIDUAL PERIOD NETWORKS
nets <- list()
years <- 2005:2016
for (yr in years) {
  yrstr <- as.character(yr)
  data_file <-  file.path(data_dir, sprintf('%s_d%s_y%s.rds',firm_i,d,yr))
  nets[[yrstr]] <- readRDS(data_file)
}
...
## SAVE SERIALIZED
fits.file <- sprintf('/home/sdowning/compnet/results/amj_rnr/fit_%s_pd%s_R%s_%s_d%s.rds', firm_i, nPeriods, R, m_x, d)
saveRDS(fits, file=fits.file)

## SAVE FORMATTED REGRESSION TABLE
html.file <- sprintf('/home/sdowning/compnet/results/amj_rnr/%s_tergm_results_pd%s_R%s_%s_d%s.html',  firm_i, nPeriods, R, m_x, d)
