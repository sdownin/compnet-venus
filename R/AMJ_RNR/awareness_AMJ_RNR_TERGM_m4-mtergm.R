...
m4_mtergm <-   nets ~ edges + gwesp(0, fixed = T) + gwdegree(0, fixed=T) + 
...
## RUN TERGM
ctrlErgm <- control.ergm(MCMC.samplesize = 5000, MCMC.interval = 2000)
fits[[firm_i]][[m_x]] <- mtergm(get(m_x), control = ctrlErgm)
