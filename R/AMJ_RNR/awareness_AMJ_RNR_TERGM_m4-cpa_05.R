...
cpc <- lapply(nets, function(net) as.matrix(net %n% 'coop') + 0.5 )
cpp <- lapply(nets, function(net) as.matrix(net %n% 'coop_past') + 0.5 )
cpa <- lapply(nets, function(net) as.matrix(net %n% 'coop') + as.matrix(net %n% 'coop_past') + 0.5  )

####################### DEFINE MODELS ###################################

m4_cpa_05 <-   nets ~ edges + gwesp(0, fixed = T) + gwdegree(0, fixed=T) + 
