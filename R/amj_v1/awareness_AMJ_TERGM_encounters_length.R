...
gs <- lapply(nets, function(net) intergraph::asIgraph(net))

## > cor(df1)
# v       n       e      dv      dn
# v   1      NA      NA      NA      NA
# n  NA  1.0000  0.9869  0.9866 -0.8176
# e  NA  0.9869  1.0000  1.0000 -0.7648
# dv NA  0.9866  1.0000  1.0000 -0.7632
# dn NA -0.8176 -0.7648 -0.7632  1.0000
##
df1 <- sapply(gs[4:length(gs)], function(g) {
  n <- igraph::vcount(igraph::induced.subgraph(g, vids=V(g)[which(igraph::degree(g)>0)]))
  ec <- ecount(g);  vc <- vcount(g)
  return(c(v=vc, n=n, e=ec, dv=round(ec/(vc*(vc-1)),4),  dn=round(ec/(n*(n-1)),4)))
})
...
# 2013    1       1      1 1.000000       1    1
# 2014    1       1      1 1.000000       1    1
# 2015    1       1      2 1.662824       2    2
# 2016    1       2      3 2.306075       3    3
# 2017    1       3      4 3.256831       4    4
# > plot(dur$Mean)
# > mean(dur$Mean)
# [1] 1.845146
##
dur <- sapply(5:length(gs), function(i){
  g <- gs[[i]]
  yr <- as.numeric(names(gs)[i])
  x2 <- sapply(E(g)$relation_ended_on, function(x)ifelse(isEmpty(x),yr,x))
  x1 <- E(g)$relation_began_on
  return(summary(1 + (x2-x1))) ## +1 --> 2014-2014 <= 1 year
})
dur <- as.data.frame(as.matrix(t(dur)))
row.names(dur) <- names(gs)[5:length(gs)]
