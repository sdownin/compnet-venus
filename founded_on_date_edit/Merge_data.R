#read data files
library(dplyr)
mydata1=read.csv(file='organizations.csv', header=T, stringsAsFactors=FALSE)
#length(unique(mydata1$com_ID)) -- # of companies in total: 433674

mydata2=read.csv(file='competitors.csv',header=T, stringsAsFactors=FALSE)
mydata3=read.csv(file='acquisitions.csv', header=T, stringsAsFactors=FALSE)
mydata4=read.csv(file='missing_companies_20180330.csv', header=T, stringsAsFactors=FALSE)
...
competition$com_founded_on[which(competition$com_founded_on=="")] <- "NA"
sum(competition$com_founded_on=="NA")
